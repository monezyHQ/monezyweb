...
export async function instantiate(imports={}, runInitializer=true) {
    ...
    const js_code = {
        ...
        'org.w3c.dom.content_$external_prop_setter' : (_this, v) => _this.content = v,
        'org.w3c.dom.HTMLMetaElement_$external_class_instanceof' : (x) => x instanceof HTMLMetaElement,
        ...
        'monezy.inapp.controller.jsShare' : 
            (title, url) => {
                if (navigator.share) {
                    try {
                         const u = new URL(url); // Validate the URL
                        navigator.share({ title: title, url: url }).catch((error) => {
                        ...
                        alert('⚠️ Invalid URL for sharing');
                        navigator.clipboard.writeText(url).then(() => {
                            alert('📋 URL copied to clipboard instead.');
                        });
                    }
                } else {
                    navigator.clipboard.writeText(url).then(() => {
        ...
        ,
        'monezy.inapp.controller.openPlayStoreLink' : (url) => { window.open(url, '_blank') }
    }
    ...
    const wasmFilePath = './MonezyApp.wasm';
    const importObject = {
        js_code,
        intrinsics: {
    ...
    try {
      ...
      if (isBrowser) {
        wasmInstance = (await WebAssembly.instantiateStreaming(fetch(new URL('./MonezyApp.wasm',import.meta.url).href), importObject)).instance;
      }
    } catch (e) {
